...
BNB_IN(2) vec2 transformed_bg_uv;
...
void main()
{
   ...
   //vec2 uvh = vec2(abs(2.0 * (var_uv_bg_uv.x - 0.5)),var_uv_bg_uv.y);
    ...
    /* calculate edge of the scaled/moved camera image */
    vec2 s_mask = step(vec2(0., 0.), transformed_bg_uv) - step(vec2(1., 1.), transformed_bg_uv);
    float edge_mask = s_mask.x * s_mask.y;

    bnb_FragColor = vec4(res, js_slider.x * edge_mask);
