...
- (void)setFeatureColorWithColor:(UIColor * _Nonnull)color;
/// \param r red channel in range [0, 1].
///
/// \param g green channel in range [0, 1].
///
/// \param b blue channel in range [0, 1].
///
/// \param a alpha channel in range [0, 1].
///
- (void)setFeatureColorWithR:(float)r g:(float)g b:(float)b a:(float)a;
...
/// \param resourcePath paths to custom resources folders
///
...
- (void)pushWithFrame:(ARFrame * _Nonnull)frame useBackCamera:(BOOL)useBackCamera SWIFT_AVAILABILITY(ios,introduced=11.0);
@end
...
- (void)willOutputWithArFrame:(ARFrame * _Nonnull)arFrame SWIFT_AVAILABILITY(ios,introduced=11.0);
@end
...
- (nonnull instancetype)initWithUseStabilization:(BOOL)useStabilization flashMode:(AVCaptureFlashMode)flashMode OBJC_DESIGNATED_INITIALIZER;
